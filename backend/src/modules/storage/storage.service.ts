...
export class StorageService {
  ...
  async uploadImage(providerId: string, fileBuffer: Buffer, filename: string) {
    ...
    const bucket =
      process.env.R2_BUCKET || process.env.R2_BUCKET_NAME || process.env.B2_BUCKET || 'hairconnekt-images';
    const key = `providers/${providerId}/${Date.now()}-${filename}`;
    const result = await this.r2.uploadObject(bucket, key, fileBuffer);
    const publicBaseUrl =
      process.env.B2_PUBLIC_BASE_URL || process.env.B2_PUBLIC_URL || process.env.R2_PUBLIC_BASE_URL || process.env.R2_PUBLIC_URL || '';
    ...
    if ((result as any)?.localPath) {
      ...
      let hostHasBucket = false;
      try {
        const u = new URL(base);
        hostHasBucket = u.hostname.startsWith(`${bucket}.`);
      } catch (_) {}
      url = hostHasBucket ? `${base}/${key}` : `${base}/${bucket}/${key}`;
    } else {
      url = `${localBase}/${bucket}/${key}`;
    }
    return { url };
